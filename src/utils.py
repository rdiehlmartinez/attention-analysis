...
def sentence_to_POS_matrix(input_toks, bias_label, indices):
    ...
    labels = []
    for i, idx in enumerate(indices): # a list of indices
        sentence_toks = input_toks[idx] # Indexing into corresponding row entry
    ...
    return DictVectorizer().fit_transform(labels)
...
def logreg_train_for_epoch(self, dataloader, input_key, label_key, **kwargs):
    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        self.model = self.model.partial_fit(inputs, labels)
        accuracies.append(self.model.score(inputs,labels))
    return accuracies
...
def logreg_binary_inference_func(self, dataloader, input_key, label_key, threshold=0.42, **kwargs):
    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        predict_probs = self.model.predict_proba(inputs, labels)
        ...
        curr_eval = {"batch_size":len(labels),
                     "accuracy":accuracy,
                     "auc":auc_score}
...
def logreg_multi_inference_func(self, dataloader, input_key, label_key, **kwargs):
    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        curr_eval = {"batch_size":len(labels), "accuracy":accuracy}
        evaluations.append(curr_eval)
    predictions = np.stack(predictions, axis=0)
    return predictions, evaluations
